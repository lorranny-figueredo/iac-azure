...
# Define um grupo de recursos chamado "projeto_iac"
resource "azurerm_resource_group" "projeto_iac" {
  name     = "Grupo_Recursos_Projeto3"
  location = "West US 2"
}

# Cria uma rede virtual chamada "iac_vnet"
resource "azurerm_virtual_network" "iac_vnet" {
  name                = "vnet_terr_iac"
  address_space       = ["10.0.0.0/16"]
  location            = azurerm_resource_group.projeto_iac.location
  resource_group_name = azurerm_resource_group.projeto_iac.name
}

# Cria a subnet dentro da rede virtual (os recursos de rede ficam na subnet)
resource "azurerm_subnet" "iac_subnet1" {
  name                 = "subnet_terr_iac"
  resource_group_name  = azurerm_resource_group.projeto_iac.name
...
resource "azurerm_network_interface" "iac_ni" {
  name                = "ni_terr_iac"
  location            = azurerm_resource_group.projeto_iac.location
  resource_group_name = azurerm_resource_group.projeto_iac.name
  ...
  ip_configuration {
    name                          = "vm_iac"
    subnet_id                     = azurerm_subnet.iac_subnet1.id
...
resource "azurerm_linux_virtual_machine" "iac_vm" {
  ...
  network_interface_ids = [azurerm_network_interface.iac_ni.id]
